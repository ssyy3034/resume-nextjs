...
// 올리브영 JD: Java/Kotlin, Spring Boot, MSA, Kafka, Redis, AWS, 대규모 트래픽, 이커머스/O2O
const oyBackend: ISkill.Skill = {
  category: 'Backend',
  items: [
    {
      title: 'Java',
    ...
    },
    {
      title: 'Redis',
      description:
        'TTL 기반 캐싱으로 API 호출 비용 절감 및 다층 캐시(Caffeine L1 + Redis L2) 구성.',
    },
    {
      title: 'RabbitMQ',
      description:
...
const oyPayload: Payload = {
  ...backendPayload,
  _global: {
    ...backendPayload._global,
    headTitle: '권동하 - CJ 올리브영 이력서',
  },
  introduce: {
    ...backendPayload.introduce,
    contents: [
      '**장애 없는 서비스를 고민하는 신입 백엔드 개발자 권동하입니다.**',

      '**트래픽 폭증에도 무너지지 않는 구조를 설계합니다**\nk6 부하 테스트에서 동시 사용자 10~20명만으로 서비스가 마비되는 장애를 발견했습니다. 원인을 Tomcat 스레드 풀 고갈로 특정하고, 무거운 연산을 RabbitMQ 비동기 워커로 격리하여 TPS 1.16 → 1,949(1,680배)로 개선했습니다. 올영세일처럼 트래픽이 급증하는 환경에서 병목을 사전에 발견하고 격리하는 것이 얼마나 중요한지 직접 체감한 경험입니다.',

      '**결제 시스템의 정합성을 코드로 검증합니다**\n토스페이먼츠 API를 연동한 결제 시스템을 설계하면서, 외부 PG 호출을 트랜잭션 밖으로 분리하여 커넥션 풀 고갈을 방지하고, 비관적 락과 멱등키로 동시성 제어를 구현했습니다. Testcontainers 기반 통합 테스트로 100개 스레드 동시 요청 환경에서 정합성 100%를 검증했습니다.',
  ...
  skill: {
    ...
    skills: [oyBackend, oyDatabase, frontend, tools],
  },
  project: {
    disable: false,
    list: [
      // StoLink: 결제 정합성, JPA 최적화, 트랜잭션 설계 강조
      {
      ...
      // Aidiary: 트래픽 대응, 비동기 아키텍처, 캐싱, 장애 격리 강조
      {
            ...
            descriptions: [
              {
                ...
                descriptions: [
                  {
                    content:
                      '**[장애 대응] 동기 블로킹 → 비동기 메시지 큐 전환으로 서비스 장애 해소**',
                    descriptions: [
                      {
                        content:
                          'k6 부하 테스트에서 동시 사용자 10~20명만으로 서비스 전체 마비 확인. ML 추론(~30초)을 Tomcat 스레드가 동기 대기하면서 스레드 풀이 고갈되는 구조',
                      },
                      {
                        content:
                          'RabbitMQ 비동기 워커 아키텍처로 무거운 연산을 격리하여 **TPS 1.16 → 1,949 (1,680배), 500 VU 부하에서 에러율 0%**',
                      },
                    ],
                  },
                  {
